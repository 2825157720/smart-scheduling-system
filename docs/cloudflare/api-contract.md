...
- 人员 GET 同时返回 `saturday_only`、`weekend_only` 和 `no_substitute` 布尔字段。三项互斥：正常前端只提交一个真值；异常冲突按“不替班”优先、其次“仅周末”、最后“仅周六”归一化。旧缓存页面在 PUT 中缺少 `weekend_only` 时，Worker 保留该人员原值，不能因编辑姓名或其他资格而误清空。
...
- 人员资格、岗位类别、仅周六、仅周末、`no_substitute`、休息/出差状态及不能替自己岗位等硬约束保持不变；“仅周末替班”只允许在周五、周六、周日作为替班候选，“仅周六替班”仍只允许周六；散排岗位仍先保留现有小组成员偏好。
...
- 自动排班先完成整岗分配，再评估允许智能拆分的岗位。每个岗位按上述公平顺序选择首个能让当天最高与最低正工作量之差至少减少 4 点的搭档；随后只应用改善最大的 1 个岗位方案。改善恰好 4 点允许拆分，低于 4 点或仅改善标准差时保持整岗；手工拆分不受此限制。
- 公平上下文仅供内部排班算法使用。`weekend_only` 通过人员 API 和 D1 `staff` schema 持久化；规则上线后不会自动改写旧有或已保存的排班，只有重新执行当天排班，或导入排休时勾选“强制重排”，才会产生新结果。
