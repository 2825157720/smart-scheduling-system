...
31 天主表、粘性表头、姓名列和拆分格保持规则几何与 0° 旋转。岗位、默认人、工作量和日期列分别固定为 108px、72px、48px 和 48px；隐藏日期不得重新分配空余宽度或拉伸剩余日期列。普通格与拆分格占满同一日期列，上午、下午槽固定各占 50%。状态底色必须配深色文字或符号；只有 `#schedule-wrap` 可以水平滚动，页面根节点不可横向溢出。
