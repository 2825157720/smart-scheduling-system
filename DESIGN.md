...
- **Dense Data**（650，10px，1）：拆分槽姓名等极高密度数据，仅在动态列宽保证完整显示时使用。
...
31 天主表、粘性表头、姓名列和拆分格保持规则几何与 0° 旋转。岗位、默认人和工作量列分别固定为 108px、72px 和 48px；日期列以 56px 为最小值，并按当前可见排班中的最长姓名统一扩宽，隐藏日期不得重新分配空余宽度或令单列独自拉伸。普通格与拆分格占满同一日期列，上午、下午槽固定各占 50%；拆分格不显示“上 / 下”标签，左槽代表上午、右槽代表下午，只显示完整姓名或状态字，并通过完整 `title` 与仅供读屏的时段说明保留语义。状态底色必须配深色文字或符号；只有 `#schedule-wrap` 可以水平滚动，页面根节点不可横向溢出。

排班格同时记录 `automatic`、`manual` 或 `legacy` 来源。岗位默认人、类别或工作量变更时，今天及未来的自动格会按当天休息名单和当前候选规则重新计算；人工格不被岗位同步覆盖。迁移前无法辨别来源的休假、替班和拆分格保持原状，并在“同步排班”预览中列出日期，只有用户明确确认后才转为自动格并重算。
